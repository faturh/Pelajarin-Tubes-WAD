...
<head>
    ...
    <title>E-Learning | Pelajarin</title>
    <style>
        /* Global Styling */
        body {
...
<body class="flex flex-col min-h-screen">
    <div class="flex-grow">
            ...
            <div class="container mx-auto flex justify-between items-center px-4 py-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-8 space-x-4 hidden md:flex">
                        ...
                        <a href="elearn" class="text-lg font-semibold text-yellow-400">E-Learning</a>
        ...
        <main class="container mx-auto px-4 py-8">
            <h1 class="text-left">Explore E-Learning Opportunities</h1>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
                @foreach ($elearns as $elearn)
                <div class="card">
                    <img src="{{ asset('storage/' . $elearn->Image) }}" alt="{{ $elearn->Title }}" class="w-full h-40 object-cover">
                    <div class="p-4">
                        <h2 class="text-lg font-bold">{{ $elearn->Title }}</h2>
                        <p class="text-sm text-gray-500">Publisher: {{ $elearn->Publisher }}</p>
                        <p class="text-sm text-gray-700 mt-2">{{ Str::limit($elearn->Description, 100) }}</p>
                        <a href="{{ route('halaman_elearn', $elearn->ElearnId) }}" target="_blank" class="btn mt-4">Learn More</a>
                    </div>
                </div>
                @endforeach
            </div>
